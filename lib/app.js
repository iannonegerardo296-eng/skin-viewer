...
async function migrate(client) {
  ...
  if (!String(schema.rows[0]?.sql || '').includes("'staff'")) {
    await client.executeMultiple(`
      ...
      INSERT INTO users_migrated (id, username, email, password_hash, role, active, created_at)
        SELECT id, username, email, password_hash, role, active, created_at FROM users;
      DROP TABLE users;
...
function validateAccountInput(body) {
  ...
  const role = ['admin', 'staff', 'user'].includes(body.role) ? body.role : 'user';
...
async function requireStaff(client, req, res) {
  const user = await sessionUser(client, req);
  if (!user) { json(res, 401, { error: 'Autenticazione richiesta' }); return null; }
  if (!['admin', 'staff'].includes(user.role)) { json(res, 403, { error: 'Accesso riservato allo staff' }); return null; }
  return user;
}
...
async function handleApi(req, res, url) {
  ...
  if (method === 'PATCH' && userPatchMatch) {
    ...
    try {
      ...
      if (body.role !== undefined && !['admin', 'staff', 'user'].includes(body.role)) { json(res, 400, { error: 'Ruolo non valido' }); return true; }
  ...
  if (method === 'GET' && route === '/api/catalog/proposals') {
    const user = await requireStaff(client, req, res); if (!user) return true;
    const result = user.role === 'admin'
      ? await client.execute(`SELECT p.id, p.name, p.image_data_url, p.model, p.tags, p.status, p.created_at, p.reviewed_at, u.username
          FROM catalog_proposals p JOIN users u ON u.id = p.user_id ORDER BY p.id DESC`)
      : await client.execute({ sql: `SELECT p.id, p.name, p.image_data_url, p.model, p.tags, p.status, p.created_at, p.reviewed_at, u.username
          FROM catalog_proposals p JOIN users u ON u.id = p.user_id WHERE p.user_id = ? ORDER BY p.id DESC`, args: [user.id] });
    json(res, 200, { proposals: result.rows.map((row) => ({
      id: Number(row.id), name: row.name, imageDataUrl: row.image_data_url, model: row.model,
      tags: JSON.parse(row.tags || '[]'), status: row.status, createdAt: row.created_at,
      reviewedAt: row.reviewed_at, username: row.username,
    })) });
    return true;
  }

  if (method === 'POST' && route === '/api/catalog/proposals') {
    const staff = await requireStaff(client, req, res); if (!staff) return true;
    try {
      const body = await readBody(req);
      const imageDataUrl = String(body.imageDataUrl || '');
      const match = imageDataUrl.match(/^data:image\/png;base64,([A-Za-z0-9+/=]+)$/);
      if (!match) { json(res, 400, { error: 'Carica un file immagine PNG valido' }); return true; }
      if (match[1].length > 400_000) { json(res, 400, { error: 'Immagine troppo grande (limite ~300 KB)' }); return true; }
      const name = String(body.name || '').trim().slice(0, 60);
      if (!name) { json(res, 400, { error: 'Inserisci un nome per la skin' }); return true; }
      const model = body.model === 'slim' ? 'slim' : 'wide';
      const tags = Array.isArray(body.tags) ? body.tags.map((tag) => String(tag).trim()).filter(Boolean).slice(0, 6) : [];
      await client.execute({
        sql: 'INSERT INTO catalog_proposals (user_id, name, image_data_url, model, tags) VALUES (?, ?, ?, ?, ?)',
        args: [staff.id, name, imageDataUrl, model, JSON.stringify(tags)],
      });
      await recordActivity(client, staff.id, 'catalog_proposal', `Proposta "${name}" inviata per approvazione`);
      json(res, 201, { message: 'Proposta inviata all’amministrazione' });
    } catch (error) { json(res, 400, { error: error instanceof Error ? error.message : 'Proposta non inviata' }); }
    return true;
  }

  const proposalReviewMatch = route.match(/^\/api\/admin\/catalog\/proposals\/(\d+)$/);
  if (method === 'PATCH' && proposalReviewMatch) {
    const admin = await requireAdmin(client, req, res); if (!admin) return true;
    try {
      const id = Number(proposalReviewMatch[1]);
      const body = await readBody(req);
      if (body.status !== 'approved' && body.status !== 'rejected') { json(res, 400, { error: 'Stato proposta non valido' }); return true; }
      const proposalResult = await client.execute({ sql: 'SELECT * FROM catalog_proposals WHERE id = ?', args: [id] });
      const proposal = proposalResult.rows[0];
      if (!proposal) { json(res, 404, { error: 'Proposta non trovata' }); return true; }
      if (proposal.status !== 'pending') { json(res, 400, { error: 'La proposta è già stata esaminata' }); return true; }
      let catalogId = null;
      if (body.status === 'approved') {
        const slug = `staff-${Date.now().toString(36)}-${crypto.randomBytes(4).toString('hex')}`;
        const inserted = await client.execute({
          sql: `INSERT INTO catalog (name, slug, source, source_url, creator, image_url, model, tags, popularity, updated_at)
                VALUES (?, ?, 'Proposta staff', '', (SELECT username FROM users WHERE id = ?), ?, ?, ?, 0, CURRENT_TIMESTAMP)`,
          args: [proposal.name, slug, proposal.user_id, proposal.image_data_url, proposal.model, proposal.tags],
        });
        catalogId = Number(inserted.lastInsertRowid);
      }
      await client.execute({ sql: 'UPDATE catalog_proposals SET status = ?, reviewed_by = ?, reviewed_at = CURRENT_TIMESTAMP WHERE id = ?', args: [body.status, admin.id, id] });
      await recordActivity(client, admin.id, `proposal_${body.status}`, `Proposta "${proposal.name}" ${body.status === 'approved' ? 'approvata e pubblicata' : 'rifiutata'}`);
      json(res, 200, { ok: true, catalogId });
    } catch (error) { json(res, 400, { error: error instanceof Error ? error.message : 'Revisione proposta non riuscita' }); }
    return true;
  }
