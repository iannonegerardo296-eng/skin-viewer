...
function requestUrl(req) {
  ...
  const normalized = parts
    ...
    .map((part) => part.trim())
    .filter(Boolean);
  if (!normalized.length && parsed.pathname.startsWith('/api/admin/catalog/')) {
    normalized.push(...parsed.pathname.slice('/api/admin/catalog/'.length).split('/').filter(Boolean));
  }
  const suffix = normalized.length ? `/${normalized.map(encodeURIComponent).join('/')}` : '';
  return new URL(`/api/admin/catalog${suffix}${parsed.search}`, `https://${host}`);
}
